...
import { ref } from 'vue'
import type { Customer, CustomerMap, DailyMetricsMap } from './storefront/types'
import {
  createCustomers,
...
const processStoreLine = () => {
  if (
    ...
    storeLog.value.push(`Customer ${getFormattedCustomer(lineCustomer)} entered the store!`)
...
const createStoreSimulation = () => {
  ...
  while (!stopSimulation.value) {
    ...
    if (customersInLine.value.length == linePos.value && customersInStore.value == 0) {
  ...
  updateDailyMetrics(
    days.value,
    { income: dailyIncome.value, totalCustomersInStore: customersInLine.value.length },
...
<template>
  <main>
    ...
    <div class="parameters">
      ...
      <input v-model="numCustomersInLine" id="line" type="number" @input="limitInputs" />
      ...
      <input
        ...
        @input="limitInputs"
      />
      <button type="button" @click="createStoreSimulation">Simulate Store</button>
    </div>
    ...
    <div>
      <table>
        ...
        <tbody>
          <tr v-for="(dailyMetrics, index) in Object.values(DAILY_METRICS_MAP)" :key="index">
            <td>{{ index + 1 }}</td>
    ...
    <div class="storefront">
      <p v-for="(storeLogEntry, index) in storeLog" :key="index">{{ storeLogEntry }}</p>
    </div>
  </main>
</template>
